...
random.seed(123)
tasks_dir = "data_generator/original_tasks"
reordered_tasks_dir = "data_generator/reordered_original_tasks"
num_test_instances = 100

test_tasks = [l.strip() for l in open(f"evaluation/test_tasks.txt")]
for task in test_tasks:
    print(f"reordering the test instances for {task}")
    file = os.path.join(tasks_dir, task + ".json")
    file_reorder = os.path.join(reordered_tasks_dir, task + ".json")
    ...
    with open(file_reorder, "w") as fout:
